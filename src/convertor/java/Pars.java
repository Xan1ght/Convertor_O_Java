...
class Pars {
   ...
   static int ConstExpr() {
      ...
      if( Scan.Lex == Scan.lexNum ) {
         ...
         if( X.Cat == Table.catGuard )
            Error.Message(
               "Нельзя определять константу через нее саму"
            );
   ...
   static int StFunc(int F) {
      ...
      return Table.typNone; // Чтобы не было предупреждений
   }

   static int Factor() {
      Obj X;
      int T = 0; // Чтобы не было предупреждений
   ...
   static int Term() {
         ...
         || Scan.Lex == Scan.lexMOD )
      {
         if( T != Table.typInt )
            Error.Message(
               "Несоответсвие операции типу операнда"
            );
         do {
            Op = Scan.Lex;
            Scan.NextLex();
   ...
   // ПростоеВыраэ [Отношение ПростоеВыраж]
   ...
   static void IfStatement() {
      ...
      if( Scan.Lex == Scan.lexELSE ) {
         Gen.Cmd(LastGOTO);   // Фиктивн. адрес, указывающий
         Gen.Cmd(OVM.cmGOTO); // на место предыдущ. перехода
         LastGOTO = Gen.PC;   // Запомнить место последн. GOTO
         Scan.NextLex();
         Gen.Fixup(CondPC);   // Зафикс. адрес усл. перехода
         StatSeq();
         }
   ...
   static void Module() {
      ...
      Converter.NameModule = ModRef.Name; //1
      Scan.NextLex();
      Check(Scan.lexSemi, "\";\"");
